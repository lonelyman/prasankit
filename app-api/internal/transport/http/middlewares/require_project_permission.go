...
// RequireProjectPermission gates a route on whether the caller's org_role in
// the resolved TenantContext is allowed for the given project permission code.
// Sister to RequireWorkspacePermission but consults project.IsRoleAllowedFor
...
func RequireProjectPermission(perm string) fiber.Handler {
	return func(c fiber.Ctx) error {
		...
		if !project.IsRoleAllowedFor(perm, tc.OrgRoleCode) {
