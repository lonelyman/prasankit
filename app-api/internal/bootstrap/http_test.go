...
func TestAuthRoutesReturnNotImplemented(t *testing.T) {
	app := newTestHTTPApp(map[string]healthCheck{
		"minio":    {},
		"postgres": {},
		"redis":    {},
	...
	tests := []struct {
		...
		{method: http.MethodPost, path: "/api/v1/auth/resend-verification-email"},
		{method: http.MethodGet, path: "/api/v1/auth/me"},
	}

	for _, tt := range tests {
		t.Run(tt.method+" "+tt.path, func(t *testing.T) {
			resp := testRequest(t, app, tt.method, tt.path)
			defer resp.Body.Close()

			if resp.StatusCode != http.StatusNotImplemented {
				t.Fatalf("status code = %d, want %d", resp.StatusCode, http.StatusNotImplemented)
			}
			...
			if gotError["code"] != "NOT_IMPLEMENTED" {
				t.Fatalf("error.code = %v, want NOT_IMPLEMENTED", gotError["code"])
			}
		})
	}
}
...
func (c healthCheck) Run(context.Context) error {
	return c.err
}
...
func newTestHTTPApp(checks map[string]healthCheck) *fiber.App {
	...
	httptransport.RegisterRoutes(
		...
		taskhttp.NewHandler(nil, nil, nil, taskhttp.CookieConfig{
			Name:     "prasankit_session",
			SameSite: "Lax",
		}),
	)
...
func (p healthCheck) PingContext(context.Context) error {
	return p.err
}
