...
## อัปเดตล่าสุด
**2026-05-31** — **M2 FE-A projects-core เสร็จ + committed** (app-web: project list/create/detail/edit/status-change/soft-delete; consume 6a project API) + **M1 FE UX polish** (dark-mode contrast/`color-scheme:light` · signup confirm-password · ws contact-prefill · home+workspaces IA redesign) + **[BE] CORS `X-Workspace-Slug` fix** (+regression test) · **D48/D49**. ผ่าน loop: 10-agent FE spec workflow → 5-OQ resolve (User, accept defaults) → external review (ณัชชา; **Kael overturn HIGH "PUT-omit-can't-clear" = false** เพราะ repo ใช้ `Updates(map)` full-replace) → Opus implementer (D37) + 5 verifier → Kael trust-but-verify (pnpm test 160 / lint / build / serve-200) + **User manual smoke** (จับ CORS bug + UX flaws ที่ automated ไม่ครอบ). **3 commit** (`877d397` FE-A+UX · `e7c5dcc` CORS · fold). ก้าวต่อ = **login→work click-reduction IA** (auto-enter single ws + `/`→`/projects` redirect + workspace controls ขึ้น header) → แล้ว **FE-B team** / **FE-C positions** ปิด M2 FE. **commit ใหม่ยัง local `dev` — ถาม User push.**
...
1. **login→work click-reduction IA — แนะนำ (User เลือกแล้ว, ทำต่อทันที)** — ปัจจุบัน login→work = 2-3 คลิก + หน้าคั่น 2 (workspaces picker + home landing). option 1: (a) **auto-enter** เมื่อมี workspace เดียว + ยังไม่เลือก (ข้าม picker) — guard ปุ่ม "เปลี่ยน Workspace" ไม่ให้ auto-bounce (เช่น sessionStorage flag); (b) **`/`→`/projects` redirect** เมื่อมี active ws (เลิก home-landing hop); (c) ย้าย workspace controls (ชื่อ/เปลี่ยน/ออกจากระบบ/เชิญ) ขึ้น **header**. เป้า: login = 1 คลิกถึง /projects. FE-only ~3 ไฟล์ (app/page.tsx, site-header.tsx, workspaces/page.tsx). main-thread iterate
