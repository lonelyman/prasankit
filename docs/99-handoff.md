...
## อัปเดตล่าสุด
**2026-06-01** — **M2 FE-B (จัดการทีม) + ปุ่มภาษา fix เสร็จ + committed + pushed `origin/dev`**. ต่อจาก FE-A/UX-slice-1: **FE-B team management** = section แรกในหน้า project detail (ตอบ "สร้างโครงการแล้วไปไหนต่อ") — **NEW BE endpoint `GET /api/v1/workspaces/members`** (repo JOIN `user_accounts` เอา display_name, isolation, active-only, gated owner/admin) + **FE Team section** (members list + role Badge · owner UUID→display_name in-place + locked · add-member **Modal** picker จาก endpoint ใหม่ · change-role inline · remove ConfirmDialog · reinstate invite Modal). lib ใหม่ `member-api.ts`/`project-roles.ts` + `workspace-api.listWorkspaceMembers`. dispatch = **pipeline workflow** (BE→FE→3 verifiers, D37 author≠reviewer; 0 HIGH/0 MED, 5 LOW non-blocking). + **[UX] ปุ่มภาษา** = segmented `[ ไทย | EN ]` + **persist `localStorage`** (เดิม refresh แล้วลืม). ผ่าน loop เต็ม: `make test -p1` เขียว (test ใหม่บน PG18) + FE **185 tests**/lint/build + **User manual smoke (สลับภาษา+refresh จำได้ · เพิ่ม bob@ เข้าโครงการสำเร็จ)**. **D51-D52.** **ก้าวต่อ = FE-C (positions)** ปิด M2 FE → แล้วขึ้น M3 (deliverables/tasks/finance). proof-of-loop เสถียร: spec/workflow → dispatch Opus D37 → verifier → Kael trust-but-verify → User manual smoke → fold+commit+push.
...
## สถานะตอนนี้
- ✅ docs ฐานครบ: [00-workflow](00-workflow.md) · [01-vision](01-vision.md) · [02-architecture](02-architecture.md) · [03-build-plan](03-build-plan.md) · [04-data-model](04-data-model.md) (รวม **§M2 ครบ — projects/team/positions/audit-by-project**) · [DECISIONS](DECISIONS.md) (**D1–D52**)
